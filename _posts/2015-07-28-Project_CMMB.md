...
关于软件开发过程中的内存泄漏，我一直认为是一个非常重要的问题，但是因为该项目是用C#编写，C#自带的内存回收机制又让我们很容易忽视掉这个问题。早期由于对此项目的认识不足，加上工程项目的复杂性，某些实例对象程序嵌套深度较广，导致在分析某些对象实例引用方面，何时不被引用，何时应该被及时释放而不驻留在内存变得困难，而由于此原因导致程序在经过长时间运行后，内存泄漏越来越严重，最终致使程序崩溃。其中让我印象深刻的，也值得重新考虑数据结构设计的，要数**功率测量部分**用于传递配置信息的结构体变量，之所以印象深刻，是因为该结构体内成员变量目前已经超过**41个，后期还要扩充**，当然，我并不清楚这个数量在
工程项目中属于哪个数量级，该结构体遍布功率测量部分，并涉及一些中间临时对象，配置信息只要一经变更，该结构体就必须重新赋值，并对图谱显示产生直接影响。因此，早期设计的时候，为了引用方便，在程序启动后就为其分配内存空间并初始化，这样做让我们不需要考虑被引用对象是否被分配内存空间，是否被初始化，可以专注于逻辑运算，表面上起到了积极效果，但是却埋下了祸根，即便某些临时对象在使用完后手动及时对其进行了释放，但是由于没有考虑到其他地方仍对该对象进行引用，导致该对象无法真正回收，因此，还需要对引用该对象的对象或者块程序进行处理，使之真正被释放。还有一点就是在模式切换后，由于大量的静态对象在程序初期被初始化，而
该部分在调制精度测量并不需要，未及时回收，也是不应该的。解决的办法一方面要优化数据结构，这一点至关重要，每一个功能子模块对应一个数据结构，保存相关信息，虽然成员变量之间可能有交错，代码量可能增加，但是以牺牲一定的时间和空间代价换取程序上的优化，是百利无一害的。另一方面在嵌套深度比较广的方法调用上，要提前梳理程序流程，定制合理接口，在对象确定不再使用后，在合适的位置将其释放，以达到内存回收的效果，当然，这只是我一面之词而已，还需要多多学习。
